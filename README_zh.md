...
你也可以尝试使用多级检索机制（感谢 Discord 上的 Kenny（深度碎片）提供思路！），把整套课程当作可查询的数据库来使用。让大模型先通过 `all.md` 判断你的问题对应课程的哪一部分；再让模型检索对应课程文件 `lessonN.md`，仅从摘要中获取答案；如果摘要信息不足，再让模型检索该课程的 CRAFT 内容得到详细解答。或者，在独立对话中使用 `add_prompt`，直接查询某一课的 CRAFT 内容，无需加载整个文件。
